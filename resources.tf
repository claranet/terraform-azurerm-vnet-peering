resource "azurerm_virtual_network_peering" "peering_src" {
  ...
  name                         = "${coalesce(var.custom_peering_dest_name, format("peering-to-%s", local.vnet_dest_name))}"
...
resource "azurerm_virtual_network_peering" "peering_dest" {
  ...
  name                         = "${coalesce(var.custom_peering_src_name, format("peering-to-%s", local.vnet_src_name))}"
